          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`group flex h-11 items-center gap-3 rounded-2xl px-3.5 text-sm font-medium transition ${active ? 'bg-[#ff7a59] text-[#141008]' : 'text-[#b8ab94] hover:bg-white/5 hover:text-[#f5edde]'}`}
              >
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <section className="admin-bottom-nav absolute inset-x-0 bottom-0 max-h-[78dvh] overflow-y-auto rounded-t-[2rem] border-t border-white/10 bg-[#1e1810] px-4 pb-4 pt-3 shadow-2xl">
            ...
            <div className="mb-3 flex items-center justify-between px-1">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#ff8f72]">Mais áreas</p>
                <h2 className="mt-1 text-xl font-semibold text-[#f5edde]">Admin +UM</h2>
              </div>
              <button onClick={() => setMoreOpen(false)} className="grid h-10 w-10 place-items-center rounded-full border border-white/8 bg-white/[0.035] text-[#b8ab94]">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {secondary.map((item) => {
                const active = isActive(item.href)
                return (
                  <Link key={item.href} href={item.href} className={`flex min-h-20 flex-col justify-between rounded-2xl border p-3 ${active ? 'border-[#ff7a59]/35 bg-[#ff7a59]/12 text-[#ff9a80]' : 'border-white/8 bg-white/[0.025] text-[#b8ab94]'}`}>
            ...
            <button onClick={signOut} className="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-2xl border border-white/8 bg-white/[0.025] text-sm font-semibold text-[#b8ab94]">
