...
- [x] Criar Edge Function `handle-payment-webhook` (AC: 2, 3, 6, 7, 8)
  - [x] Verificar assinatura HMAC do webhook (AC: 7)
  - [ ] Handler para `subscription.created`: inserir subscription com status inicial
  - [x] Handler para `subscription.paid`: atualizar status, registrar payment, chamar `allocate_coupons` (AC: 6)
  - [x] Handler para `subscription.cancelled`: atualizar status para `cancelled`
  - [x] Handler para `subscription.failed`: atualizar status para `past_due`, enviar push ao usuario
  - [x] Logging de todos os eventos recebidos (AC: 8)
